...
import {
  UserType,
  LocationType,
  LocationUpdateInputType,
  LocationNameType,
  LocationNameInputType,
  RecommendedLocationType,
} from "./graphQLObjects.js";
...
const RootQueryType = new GraphQLObjectType({
  ...
  fields: () => ({
    ...
    userByField: {
      type: new GraphQLList(UserType),
      description:
        "Returns users matching a partial search from a specific field",
      args: {
    ...
    userByID: {
      type: UserType,
      description:
        "Returns the user document with the given MongoDB Object ID field",
      args: {
    ...
    recommendedLocations: {
      ...
      description:
        "Returns a list of num_recommendations number of recommended locations for the user specified by user_id",
      args: {
        user_id: { type: GraphQLNonNull(GraphQLString) },
        num_recommendations: { type: GraphQLInt },
      },
      resolve: (parent, args) =>
        RecommendLocations(args.user_id, args.num_recommendations),
    },
  }),
});
...
const RootMutationType = new GraphQLObjectType({
  ...
  fields: () => ({
    addUser: {
      type: GraphQLString,
      description:
        "Add a new non existing user to the users collection of the db",
      args: {
        email: { type: GraphQLNonNull(GraphQLString) },
        username: { type: GraphQLNonNull(GraphQLString) },
      ...
      resolve: (parent, args) =>
        AddUser(args.email, args.username, args.password),
    },
    login: {
      type: GraphQLString, // You can use a String for returning a token or a session
      description:
        "Authenticate User Login and return the Object ID on sucessful login",
      args: {
    ...
    addLocation: {
      type: GraphQLString,
      description:
        "Add a new location (won't accept any location that is in a 5km radius from an already existing location)",
      args: {
        user_id: { type: GraphQLNonNull(GraphQLString) },
      ...
      resolve: (parent, args) =>
        addLocation(args.user_id, args.name, args.latitude, args.longitude),
    },
    updateLocation: {
      type: LocationType,
    ...
    deleteLocation: {
      type: GraphQLString,
      description:
        "Remove entry based on the Object ID of the selected location",
      args: {
        _id: { type: GraphQLString },
      },
